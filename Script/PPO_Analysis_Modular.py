...
# ──────────────────────────────────────────────────────────────────
    ...
    # helper ───────────────
    def _pareto_mask(df: pd.DataFrame, reward_col: str, stab_col: str) -> pd.Series:
        """
        Return Boolean mask of Pareto-optimal rows (higher reward, lower stability).
        """
        mask = []
        for i, row_i in df.iterrows():
            ...
            for j, row_j in df.iterrows():
                if j == i:
                    continue
                better_or_equal = (row_j[reward_col] >= row_i[reward_col]) and \
                                (row_j[stab_col]   <= row_i[stab_col])
                strictly_better = (row_j[reward_col] >  row_i[reward_col]) or \
                                (row_j[stab_col]   <  row_i[stab_col])
    ...
    # main ────────────────
    def make_pareto_plots(csv_path: str,
                        save_dir: str,
                        config) -> None:
        """
        Create 4 Pareto charts:
        • one global (all climates pooled)
        • one for each climate (Cool, Hot, Mix)
        All styling matches create_enhanced_descriptives().
        """
        # ── load & tidy ────────────────────────────────────────────────
        df = (pd.read_csv(csv_path)
                .rename(columns={
                    "RewardZ_mean_post": "Reward",
                    "logCVZ_mean_post": "Stability"})
                .assign(LearningRate=lambda d:
                        d["Learning Rate"].map({
                            'Fixed Moderate (η = 0.0005)': 'LR00005',
                            'Fixed High (η = 0.001)':      'LR0001',
                            'Adaptive Decay (η = 0.001 → 0.00001)': 'LateDecayLR'
                        }))
                .assign(LR_Short=lambda d:
                        d["LearningRate"].map(config.LR_MAPPING_SHORT)))

        # marker & colour dicts
        lr_marker = {'LR00005': 'o', 'LR0001': 'X', 'LateDecayLR': 'D'}
        climate_cols = config.CLIMATE_COLORS

        # legend handles
        lr_handles = [Line2D([0], [0], marker=m, color='grey',
                            label=config.LR_MAPPING[lr].replace('\n', ' '),
                            markersize=10, linestyle='None')
                    for lr, m in lr_marker.items()]
        clim_handles = [Patch(facecolor=climate_cols[c], label=c) for c in ['Cool','Hot','Mix']]

        # plotting helper
        def _single_ax(ax, sub_df, title):
            # scatter
            for _, r in sub_df.iterrows():
                ax.scatter(r.Reward, r.Stability,
                        color=climate_cols[r.Climate],
                        marker=lr_marker[r.LearningRate],
                        s=200, edgecolors='black', linewidth=1.2)
            # pareto ring
            pareto_pts = sub_df[_pareto_mask(sub_df, "Reward", "Stability")]
            ax.scatter(pareto_pts.Reward, pareto_pts.Stability,
                    facecolors='none', edgecolors='black',
                    s=450, linewidth=3)
            # labels, grid
            ax.set_title(title, fontsize=16, fontweight='bold', pad=20)
            ax.set_xlabel("Reward (Z-score; higher = better)",
                        fontsize=12, fontweight='bold')
            ax.set_ylabel("log-CV (Z-score; lower = better)",
                        fontsize=12, fontweight='bold')
            ax.grid(True, alpha=0.3, linestyle='-', linewidth=0.5)
        ...
        # global
        _single_ax(axes[0], df, "All climates pooled")
        # split per climate
        for i, clim in enumerate(['Cool','Hot','Mix'], start=1):
            _single_ax(axes[i],
                    df[df.Climate == clim],
                    f"{clim} climate")

        # legends (placed once)
        axes[0].legend(handles=clim_handles,
                    title="Climate", loc='upper left',
                    frameon=True, fancybox=True, shadow=True,
                    fontsize=10, title_fontsize=11)
        axes[0].add_artist(
            axes[0].legend(handles=lr_handles,
        ...
        plt.tight_layout()
        out_path = pathlib.Path(save_dir)/"pareto_reward_stability.png"
        plt.savefig(out_path, dpi=300, bbox_inches='tight', facecolor='white')
        plt.close()
        print(f"✓ Pareto plots saved to {out_path}")



    def _create_rank_grid(self, results_df: pd.DataFrame) -> None:
        """Create 3x3 rank grid showing reward rank / stability rank."""
        # Compute ranks within each climate
        results_df['Reward_Rank'] = results_df.groupby('Climate')['Reward'].rank(
            ascending=False, method='min').astype(int)  # Higher reward = better rank (1)
        results_df['Stability_Rank'] = results_df.groupby('Climate')['Stability'].rank(
            ascending=True, method='min').astype(int)   # Lower log-CV = better rank (1)
        
        # Create 3x3 grid
        fig, axes = plt.subplots(nrows=3, ncols=3, figsize=(10, 10))
        
        lr_order = ['Moderate', 'High', 'Decay']
        climate_order = ['Cool', 'Hot', 'Mix']
        
        for row_idx, lr in enumerate(lr_order):
            for col_idx, climate in enumerate(climate_order):
                ax = axes[row_idx, col_idx]
                
                # Find the corresponding data point
                data_point = results_df[
                    (results_df['LR_Short'] == lr) & 
                    (results_df['Climate'] == climate)
                ].iloc[0]
                
                r_rank = data_point['Reward_Rank']
                s_rank = data_point['Stability_Rank']
                
                # Display "R/S" in the center of each tile
                ax.text(0.5, 0.5, f"{r_rank}/{s_rank}", 
                ...
                if row_idx == 0:
                    ax.set_title(climate, fontsize=14, fontweight='bold', pad=10)
                
                # Row labels (left column only)
                if col_idx == 0:
                    ax.set_ylabel(lr, fontsize=14, fontweight='bold', rotation=90, labelpad=15)
        
        # Overall title
        fig.suptitle("Rank Grid (Reward rank / Stability rank)", 
                    fontsize=16, fontweight='bold', y=0.95)
        
        # Set axis labels for the entire grid
        fig.text(0.5, 0.02, 'Climate', ha='center', va='bottom', 
                fontsize=14, fontweight='bold')
        fig.text(0.02, 0.5, 'Learning Rate', ha='left', va='center', 
        ...
        filename = "rank_grid_reward_stability.png"
        plt.savefig(self.config.OUT_FIG / filename, dpi=300, 
